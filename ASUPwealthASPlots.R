# spits out survey-weighted wealth histograms and active saving plots (actual and logs)
#to /Users/Rachel/Documents/PhD/PhD\ Thesis/Chapter2/Figs/Raster
# need to run load.R, ASUPconstructFamPan.R and ASUPwideToLong.R first
...
for(i in head(years, n=length(years))){
  ...
  svyhist(~eval(as.name("impWealthWOE")),
          ...
          main="", col="grey80", xlab=paste("Wealth",shortYeari), breaks=100, cex.axis=0.75, cex.lab=0.75)
  
}
setwd("/Users/Rachel/Documents/PhD/PhD\ Thesis/Chapter2/Figs/Raster")
dev.copy(png,'wealthHistogram.png')
dev.off()
...
for(i in head(years, n=length(years)-2)){
  ...
  tempj <- rename(tempj, c('activeSaving'=paste('activeSaving',shortYearj,sep=''),
                           'hhHead'=paste('hhHead',shortYearj,sep='')))
  # yeark
...
# save file
setwd("/Users/Rachel/Documents/PhD/PhD\ Thesis/Chapter2/Figs/Raster")
dev.copy(png,'activeSavingLogs.png')
dev.off()
setwd("/Users/Rachel/Documents/PhD/Research ideas/Nonlinearity/PSID/ActiveSaving/")
